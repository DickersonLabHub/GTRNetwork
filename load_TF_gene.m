function [ TF_gene] = load_TF_gene( filename,sigma_file )
...
if l~=-1
    ...
    TF_gene.Gene_id(G,1:2)=[s(4,1) s(5,1)];
    TF_gene.TF_id(T,1:3)=s(1:3,1)';
    TF_gene.AI(N,1:5)=s([1 4 2 6 8],1)';
    TF_gene.C(N,G)=1;
...
while (infile)
    ...
    if l~=-1
       s=strread(l,'%s','delimiter','\t');
        ...
        TF_gene.AI(N,1:5)=s([1 4 2 6 8],1)';
        fprintf('%s%i\n','Loading TF gene link number ',N);
       g=strmatch(s(4,1),TF_gene.Gene_id(:,1),'exact');
       if isempty(g)
           G=G+1;
           g=G;
           TF_gene.Gene_id(G,1:2)=[s(4,1) s(5,1)];
